...
getWeightObservations <- function( username, fitbitkey, fitbitsecret, fitbitappname) {
  ...
  token <- readRDS(paste("weightwatcher/users/", username, "/fitbit-token.RDS", sep=""))
...
buildNudgeRequest <- function( userid, username, DF ) {
  ...
  # Participants
  factid <- 0
  fileName <- paste('weightwatcher/users/', username, '/Participant/fact.xml', sep="");
  fact <- readChar( fileName, file.info(fileName)$size )
  factbody <- paste(factbody, fact, sep=" ")
  
  # Goals
  fileName <- paste('weightwatcher/users/', username, '/Goal/fact.xml', sep="");
  fact <- readChar( fileName, file.info(fileName)$size )
  factbody <- paste(factbody, fact, sep=" ")
  
  # Observations
  factid <- 0
  fileName <- paste('weightwatcher/templates/fact.xml');
  ...
  # Envelope
  fileName <- 'weightwatcher/templates/fact-envelope.xml';
